...
public class FtpClientUtils {
	...
	public boolean moveDir(String src, String des) {
		List<FileForFTP> fileForFTPs = new ArrayList<FileForFTP>();
		fileForFTPs = this.getFiles(fileForFTPs, src);
		for (FileForFTP fileForFTP : fileForFTPs) {
			this.moveFile(fileForFTP.getFilePath() + "/" + fileForFTP.getFtpFile().getName(), fileForFTP.getFilePath().replace(src, des) + "/"
					+ fileForFTP.getFtpFile().getName());
		}
		return true;
	}
